...
{
	std::string m_uniformNameVerbatim;
...
public:
	...
	static const std::map< std::string, ShaderProgram* >* GetRegistry() { return &s_shaderProgramRegistry; }
...
//	void RecordInputAttributes(); //Handled by Material::BindInputAttribute() as called by MeshRenderer::SetMaterial() as it needs the MR's VAO ID.
	...
	unsigned int BindInputAttribute( const std::string& attributeNameVerbatim, unsigned int count, unsigned int glFieldType, bool normalize, int strideInBytes, int offsetInBytes );
	...
	bool SetVector2( const std::string& uniformNameVerbatim, const Vector2* newValue, unsigned int arraySize = 1 );
	bool SetVector3( const std::string& uniformNameVerbatim, const Vector3* newValue, unsigned int arraySize = 1 );
	bool SetVector4( const std::string& uniformNameVerbatim, const Vector4* newValue, unsigned int arraySize = 1 );
	bool SetMatrix4x4( const std::string& uniformNameVerbatim, bool shouldTranspose, const Matrix4x4* newValue, unsigned int arraySize = 1 );
