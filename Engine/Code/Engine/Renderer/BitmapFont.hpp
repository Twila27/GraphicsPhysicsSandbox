...
public:
	...
	AABB2 GetTexCoordsForGlyph( int glyphUnicode ) const; //Can't rely on SpriteSheet now, need to call corresponding Glyph object.
...
private:
	...
	static std::map< std::string, BitmapFont* > s_fontRegistry;	//Uses .fnt filepath as a key--only ever one .fnt file even for multi-page bitmap fonts.
