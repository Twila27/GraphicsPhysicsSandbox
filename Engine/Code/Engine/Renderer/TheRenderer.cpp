...
void TheRenderer::CreateBuiltInTextures()
{
	...
	m_defaultFont = BitmapFont::CreateOrGetFont( "Data/Fonts/Input.fnt" );
	...
	: m_currentFBO( nullptr )
{
	...
	CreateBuiltInTextures(); //Ensure this is first to give defaults the first possible texture IDs.
}
...
//--------------------------------------------------------------------------------------------------------------
void TheRenderer::DrawTextInBox2D( const AABB2& textboxBounds, const Rgba& textboxColor, const std::string& /*text*/, 
...
void TheRenderer::DrawTextProportional3D( const Vector3& lowerLeftOriginPos, const std::string& inputText, const Vector3& textPlaneUpDir, const Vector3& textPlaneRightDir, 
										  float scale /*= .25f*/, const BitmapFont* font /*= nullptr*/, const Rgba& tint /*= Rgba()*/, bool drawDropShadow /*= true*/, const Rgba& shadowColor /*=Rgba::BLACK*/ )
{
	...
	Vector3 cursor = lowerLeftOriginPos; //Assuming its the lower-left.
	...
	for ( unsigned int charIndex = 0; charIndex < inputText.size(); charIndex++ )
	{
		...
		const Vector2 offset = Vector2( static_cast<float>( currentGlyph->m_xoffset ), static_cast<float>( currentGlyph->m_yoffset ) ); //"caching" off is good!
		const Vector2 size = Vector2( static_cast<float>( currentGlyph->m_width ), static_cast<float>( currentGlyph->m_height ) ); //in px, not uv == normalized texel coordinates

		//Remember: in 2D, y+ is up, x+ is right. But positive yoffset means DOWN, though positive xoffset still means right.
		const Vector3 topLeftCorner = cursor + ( textPlaneRightDir * offset.x * scale ) + ( textPlaneUpDir * -offset.y * scale ); //-?
		const Vector3 bottomLeftCorner = topLeftCorner - ( textPlaneUpDir * size.y * scale );
		const Vector3 bottomRightCorner = bottomLeftCorner + ( textPlaneRightDir * size.x * scale );
		const Vector3 topRightCorner = topLeftCorner + ( textPlaneRightDir * size.x * scale );

		AABB3 renderBounds = AABB3( bottomLeftCorner, topRightCorner );

		AABB2 texCoords = font->GetTexCoordsForGlyph( currentGlyph->m_id );

		Texture* texture = font->GetFontTexture( currentGlyph->m_page );

		if ( drawDropShadow )
		{
			AABB3 shadowRenderBounds = renderBounds;
			Vector3 shadowOffset = Vector3( DROP_SHADOW_OFFSET, -DROP_SHADOW_OFFSET, -DROP_SHADOW_OFFSET );
...
//--------------------------------------------------------------------------------------------------------------
void TheRenderer::DrawTextProportional2D( const Vector2& lowerLeftOriginPos, const std::string& inputText, float scale /*= .25f*/, const BitmapFont* font /*= nullptr*/, const Rgba& tint /*= Rgba()*/, bool drawDropShadow /*= true*/, const Rgba& shadowColor /*=Rgba::BLACK*/ )
{
	if ( font == nullptr )
		font = m_defaultFont;

	Vector2 cursor = lowerLeftOriginPos; //Assuming its the lower-left.
	...
	for ( unsigned int charIndex = 0; charIndex < inputText.size(); charIndex++ )
	{
		...
		//Remember: in 2D, y+ is up, x+ is right. But positive yoffset means DOWN, though positive xoffset still means right.
		const Vector2 topLeftCorner			= Vector2( cursor.x + offset.x * scale, cursor.y - offset.y * scale );
		const Vector2 bottomLeftCorner		= Vector2( topLeftCorner.x,							topLeftCorner.y - ( size.y * scale ) );
		const Vector2 bottomRightCorner		= Vector2( bottomLeftCorner.x + ( size.x * scale ), bottomLeftCorner.y );
		const Vector2 topRightCorner		= Vector2( topLeftCorner.x + ( size.x * scale ),	topLeftCorner.y );

		AABB2 renderBounds = AABB2( bottomLeftCorner, topRightCorner );
		...
		if ( drawDropShadow )
		{
			...
			Vector2 shadowOffset = Vector2( DROP_SHADOW_OFFSET, -DROP_SHADOW_OFFSET );
			shadowRenderBounds.mins += shadowOffset;
			shadowRenderBounds.maxs += shadowOffset;
			g_theRenderer->DrawAABB( VertexGroupingRule::AS_QUADS, shadowRenderBounds, *texture, texCoords, shadowColor );
		}
...
//--------------------------------------------------------------------------------------------------------------
void TheRenderer::DrawTextMonospaced2D( const Vector2& originPos, const std::string& inputText, float cellHeight, const Rgba& tint /*= Rgba()*/, const BitmapFont* font /*= nullptr*/, float cellAspect /*= 1.f */, bool drawDropShadow /*= true*/ )
{
	if ( font == nullptr ) 
		font = m_defaultFont;

	Vector2 glyphBottomLeft = originPos; //Treated as the upper/lower-left.
	Vector2 cellSize( cellAspect * cellHeight, cellHeight );

	for ( int stringIndex = 0; stringIndex < (int)inputText.size( ); stringIndex++ )
	{
		AABB2 texCoords = font->GetTexCoordsForGlyph( inputText[ stringIndex ] ); //this is returning (topLeftX, topLeftY, bottomRightX, bottomRightY).
...
//--------------------------------------------------------------------------------------------------------------
void TheRenderer::DrawCloudySphere( Vector3 position, float radius, float numPoints, const Rgba& tint /*= Rgba()*/ )
{
...
//--------------------------------------------------------------------------------------------------------------
void TheRenderer::DrawSphereGimbal( const int vertexGroupingRule, const Vector3& centerPos, float radius, float numSides, const Rgba& tint /*= Rgba()*/, float lineThickness /*= 1.0f*/ )
{
	TODO( "Move the code from SD3's A3's Main_Win32 into here to draw correct UV sphere." );
