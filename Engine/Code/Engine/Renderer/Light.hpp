...
public:
	...
	static Light* CreateLight( LightType type, const Vector3& position, const Vector3& direction, const Rgba& color,
	...
	static const std::vector<Light*>* GetRegistry() { return &s_lightRegistry; }

	static void GetLightPositions( Vector3* out );
	static void GetLightDirections( Vector3* out );
...
private:
	...
	Light( LightType type, const Vector3& position, const Vector3& direction, const Rgba& color,
		   float minLightDistRadius = 0.f, float maxLightDistRadius = INFINITY,
		   float powerAtMinDist = 0.f, float powerAtMaxDist = 0.f, 
		   float innerAngleCosine = 0.f, float outerAngleCosine = 0.f, 
		   float powerInsideInnerAngle = 0.f, float powerOutsideOuterAngle = 0.f );

	void MakePointLight( const Vector3& position, const Vector3& direction, const Rgba& color, float minLightDistRadius, float maxLightDistRadius );
	void MakeSpotLight( const Vector3& position, const Vector3& direction, const Rgba& color, float minLightDistRadius, float maxLightDistRadius,
	...
	void MakeDirectionalLight( const Vector3& position, const Vector3& direction, const Rgba& color );

	static std::vector<Light*> s_lightRegistry;

	bool m_isLightOn;
	Vector3 m_position; //"P"
	Vector3 m_direction; //"L_d" the direction from light, but use -L_d in code for direction to light.
	Rgba m_color;
