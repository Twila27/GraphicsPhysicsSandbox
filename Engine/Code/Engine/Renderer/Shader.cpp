...
GLint LoadShader( const char* filePath, GLenum shaderType )
{
	...
	GLint srcLength = shaderSourceBuffer.size();
	...
	if ( status == GL_FALSE )
	{
		...
		ReportShaderCompileError( filePath, errorBuffer );
...
{
	if ( s_shaderRegistry.find( shaderFilePath ) != s_shaderRegistry.end() ) return s_shaderRegistry[ shaderFilePath ];

	unsigned int shaderID = LoadShader( shaderFilePath.c_str(), GetOpenGLShaderType( shaderType ) );
	s_shaderRegistry[ shaderFilePath ] = new Shader( shaderFilePath, shaderType, shaderID );
