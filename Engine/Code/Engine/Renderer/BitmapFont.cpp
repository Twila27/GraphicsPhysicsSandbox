...
BitmapFont::~BitmapFont()
{
	for ( int pageIndex = 0; pageIndex < m_numPages; pageIndex++ )
	{
		...
		{
			delete m_fontAtlases[ pageIndex ];
...
void BitmapFont::LoadFontImage( const std::string& imageFilePath, unsigned int pageIndex )
{
	...
	IntVector2 imageSizeInTexels;
	int numChannels;
	int numChannelsRequested = 0; //Means we don't care.
	unsigned char* undupedImageData = stbi_load( imageFilePath.c_str(), &imageSizeInTexels.x, &imageSizeInTexels.y, &numChannels, numChannelsRequested );
	...
	unsigned char* dupedImageData = new unsigned char[ numBytes * 4 ]; //Duplicate font image's 1-channel "R" data across 4-channels "Rgba".
...
{
	if ( s_fontRegistry.find( fntFilePath ) != s_fontRegistry.end( ) ) return s_fontRegistry[ fntFilePath ];
	else s_fontRegistry[ fntFilePath ] = new BitmapFont( fntFilePath );
	
	return s_fontRegistry[ fntFilePath ];
}
...
AABB2 BitmapFont::GetTexCoordsForGlyph( int glyphUnicode ) const
{
	...
	return AABB2( topLeftX, topLeftY, bottomRightX, bottomRightY );
}
