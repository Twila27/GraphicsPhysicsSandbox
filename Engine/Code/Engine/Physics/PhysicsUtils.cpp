...
void Particle::Render()
{
	...
	case PARTICLE_SPHERE:
		g_theRenderer->DrawCloudySphere( m_state->GetPosition(), m_renderRadius, 20.f );
		break;
	case PARTICLE_AABB3:
	...
	if ( m_secondsPassedSinceLastEmit >= m_secondsBetweenEmits )
	{
		...
		for ( unsigned int iterationNum = 0; iterationNum < m_particlesEmittedAtOnce; iterationNum++ )
		{
			...
			muzzleVelocity.x = m_muzzleSpeed
				* SinDegrees( ( spanDegreesDownFromWorldUp		* GetRandomFloatFromZeroTo( 1.0f ) ) + m_minDegreesDownFromWorldUp )
				* CosDegrees( ( spanDegreesLeftFromWorldNorth	* GetRandomFloatFromZeroTo( 1.0f ) ) + m_minDegreesLeftFromWorldNorth );
			muzzleVelocity.y = m_muzzleSpeed
				* SinDegrees( ( spanDegreesDownFromWorldUp		* GetRandomFloatFromZeroTo( 1.0f ) ) + m_minDegreesDownFromWorldUp )
				* SinDegrees( ( spanDegreesLeftFromWorldNorth	* GetRandomFloatFromZeroTo( 1.0f ) ) + m_minDegreesLeftFromWorldNorth );
			muzzleVelocity.z = m_muzzleSpeed
				* CosDegrees( ( spanDegreesDownFromWorldUp		* GetRandomFloatFromZeroTo( 1.0f ) ) + m_minDegreesDownFromWorldUp ); //Embeds assumption z is world-up? Would it work if using y-up, just rotated by 90deg?
