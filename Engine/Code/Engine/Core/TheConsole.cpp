...
//--------------------------------------------------------------------------------------------------------------
void TheConsole::Printf( const char * format, ... )
{
	const int STRINGF_STACK_LOCAL_TEMP_LENGTH = 2048;

	//Duplicating code from Stringf() because it appears I'd need to do the same thing just to pass it to Stringf().
	...
	m_storedText.push_back( std::pair< std::string, Rgba>( std::string(textLiteral), m_currentColor ) );
}
...
void TheConsole::Render() //Recall up is +y.
{
	...
	m_currentPromptBoxTopLeft.x = spacingPx;
	m_currentPromptBoxTopLeft.y = spacingPx + heightOfOneLinePx;
	Vector2 currentPromptBoxBottomRight = Vector2( (float)m_screenWidth - spacingPx, spacingPx );

	//Sizes for the log: for each stored line of text add heightOfOneLinePx under the max, at which point cut the log off.
	Vector2 currentLogBoxBottomRight = Vector2( currentPromptBoxBottomRight.x, m_currentPromptBoxTopLeft.y + ( spacingPx * 2.f ) );
	...
	{
		if ( totalHeightOfLinesInLogPx + heightOfOneLinePx < m_maxConsoleHeight )
			totalHeightOfLinesInLogPx += heightOfOneLinePx;
		else break;
	}
	m_currentLogBoxTopLeft = Vector2( m_currentPromptBoxTopLeft.x, currentLogBoxBottomRight.y + totalHeightOfLinesInLogPx + spacingPx );

	//Background first: just one amount of spacing above the top of the text entry log's box.
	g_theRenderer->DrawAABB( TheRenderer::AS_QUADS, AABB2( Vector2( 0, m_currentLogBoxTopLeft.y + spacingPx ), Vector2( (float)m_screenWidth, 0.f ) ), bgColor );

	//Then the text fields.
	g_theRenderer->DrawShadedAABB( TheRenderer::AS_QUADS, AABB2( m_currentLogBoxTopLeft, currentLogBoxBottomRight ), bgColor, bgColor, fgColor, fgColor );
	g_theRenderer->DrawAABB( TheRenderer::AS_QUADS, AABB2( m_currentPromptBoxTopLeft, currentPromptBoxBottomRight ), fgColor );

	//Actual input text so far.
	g_theRenderer->DrawTextProportional2D( Vector2( spacingPx * 2.f, m_currentPromptBoxTopLeft.y - spacingPx ), m_currentPromptString, m_currentScale, m_currentFont, m_currentColor );

	//"Blinking" cursor.
	Rgba caretColor = Rgba( m_currentColor.red, m_currentColor.green, m_currentColor.blue, static_cast<unsigned char>( RangeMap( m_caretAlphaCounter, 0.f, 1.f, 0.f, 255.f ) ) );
	float caretX = g_theRenderer->CalcTextPxWidthUpToIndex( m_currentPromptString, m_caretPosInInputString ); //Draw blinking underscore here.
	if ( m_caretPosInInputString == (int)m_currentPromptString.size() )
		g_theRenderer->DrawTextProportional2D( Vector2( caretX, heightOfOneLinePx ), "_", m_currentScale, m_currentFont, caretColor );
	else
		g_theRenderer->DrawTextProportional2D( Vector2( caretX, heightOfOneLinePx ), "|", m_currentScale, m_currentFont, caretColor );
	...
	Vector2 positionInLog = Vector2( spacingPx * 2.f, m_currentLogBoxTopLeft.y - spacingPx );
...
{
	if ( g_theInput->WasKeyJustPressed( KEY_TO_OPEN_CONSOLE ) )
