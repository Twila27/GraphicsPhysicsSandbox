	...
	: m_currentArgsListPos( 0 )
{
	...
	//WARNING: if the args contain chars not in this regex, will cut off!!!
	int scanForArgsResult = sscanf_s( fullCommandString.c_str(), "%*s %[0-9a-zA-Z.- ]", argsString, _countof(argsString) );
...
//--------------------------------------------------------------------------------------------------------------
bool Command::GetNextString( std::string* out, const std::string* defaultValue )
{
	std::string arg;
